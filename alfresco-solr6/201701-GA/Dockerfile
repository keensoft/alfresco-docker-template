...
ENV SOLR6_DOWNLOAD_URL https://download.alfresco.com/release/community/201701-build-00015/alfresco-search-services-1.0.b.zip
ENV SOLR_DIR /usr/local/solr

RUN set -x \
	&& mkdir -p $SOLR_DIR \
	&& wget --no-check-certificate $SOLR6_DOWNLOAD_URL \
	&& unzip alfresco-search-services-1.0.b.zip -d $SOLR_DIR \
	&& rm -f alfresco-search-services-1.0.b.zip
